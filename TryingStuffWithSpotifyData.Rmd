...
both <- top_songs %>% left_join(top_artists, by = "artist") %>% 
  filter(total_mins > 165)

both %>% ggplot(aes(big_artists, total_mins, label = title, color = artist)) + 
  geom_point() +
  geom_label_repel(hjust=1, vjust=0.5, size = 2, alpha = 0.8) +
  theme_classic() +
  labs(
    title = "Most popular songs by total listening to artists",
    subtitle = "Drake dominates the listening by specific artist",
